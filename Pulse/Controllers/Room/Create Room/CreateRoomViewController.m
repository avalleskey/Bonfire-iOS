...
- (void)viewDidLoad {
    ...
    // Google Analytics
    [FIRAnalytics setScreenName:@"Create Room" screenClass:nil];
    self.createTrace = [FIRPerformance startTraceWithName:@"Create Room"];
}
...
- (void)roomUpdated:(NSNotification *)notification {
    ...
    if (room != nil) {
        for (NSInteger i = 0; i < self.similarRooms.count; i++) {
...
- (void)setupViews {
    ...
    [self.closeButton bk_whenTapped:^{
        [FIRAnalytics logEventWithName:@"abort_create_room"
...
- (void)setupSteps {
    ...
    [self.steps addObject:@{@"id": @"room_share", @"skip": [NSNumber numberWithBool:false], @"next": @"Enter Camp", @"instruction": @"Your Camp has been created!\nInvite others to join below", @"sensitive": [NSNumber numberWithBool:true], @"answer": [NSNull null], @"block": [NSNull null]}];
...
- (void)setColor:(UIView *)sender {
    if (sender.tag != self.themeColor) {
        ...
        NSInteger colorStep = [self getIndexOfStepWithId:@"room_color"];
...
- (void)getSimilarRooms {
    NSInteger roomNameIndex = [self getIndexOfStepWithId:@"room_name"];
    NSString *roomName = ((UITextField *)self.steps[roomNameIndex][@"textField"]).text;
    NSInteger roomDescriptionIndex = [self getIndexOfStepWithId:@"room_description"];
    ...
    [[HAWebService authenticatedManager] GET:@"search/rooms" parameters:@{@"q": query} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        NSInteger similarRoomsStepIndex = [self getIndexOfStepWithId:@"room_similar"];
...
- (void)createRoom {
    NSInteger titleStep = [self getIndexOfStepWithId:@"room_name"];
    UITextField *titleTextField = self.steps[titleStep][@"textField"];
    NSString *roomTitle = titleTextField.text;
    
    NSInteger descriptionStep = [self getIndexOfStepWithId:@"room_description"];
    ...
    NSInteger themeAndPrivacyStep = [self getIndexOfStepWithId:@"room_color"];
    ...
    [[HAWebService authenticatedManager] POST:@"rooms" parameters:@{@"title": roomTitle, @"description": roomDescription, @"color": roomColor, @"visibility": [NSNumber numberWithBool:visibility]} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSError *error;
        Room *room = [[Room alloc] initWithDictionary:responseObject[@"data"] error:&error];
        
        // update share url
        NSInteger shareStep = [self getIndexOfStepWithId:@"room_share"];
        UIView *shareBlock = self.steps[shareStep][@"block"];
        UIButton *shareField = [shareBlock viewWithTag:10];
        [shareField setTitle:[NSString stringWithFormat:@"https://bonfire.camp/c/%@", room.attributes.details.identifier] forState:UIControlStateNormal];
        ...
        // Google Analytics
        [FIRAnalytics logEventWithName:@"room_created"
                            parameters:@{
