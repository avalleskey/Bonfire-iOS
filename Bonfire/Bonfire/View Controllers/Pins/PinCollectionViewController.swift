...
final class PinCollectionViewController: UICollectionViewController {
    ...
    init() {
        flow.sectionInset = .init(top: 24,
                                  left: 20,
                                  bottom: 20,
                                  right: 20)
        super.init(collectionViewLayout: flow)
        collectionView.register(PinCollectionViewCell.self,
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: PinCollectionViewCell.reuseIdentifier,
        ...
        case is User:
            guard let user = pin.object as? User,
                  let imageURL = user.attributes.media?.avatar?.full?.url else { return cell }
...
extension PinCollectionViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        let width: CGFloat = collectionView.bounds.width/3.0 - flow.sectionInset.left - flow.sectionInset.right
