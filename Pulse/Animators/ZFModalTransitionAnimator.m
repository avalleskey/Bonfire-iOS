...
@property CGAffineTransform tempTransform;
@end
...
- (void)setup {
    ...
    _dragable = NO;
    _bounces = YES;
    _behindViewScale = 0.82;
    _behindViewAlpha = 0.5;
    _transitionDuration = 0.6f;
    _dismissVelocity = 100.0f;
    _dismissDistance = 100.0f;
    
    [self setContentScrollView:[self viewControllerScrollView]];
...
- (UIScrollView *)viewControllerScrollView {
    UIView *viewToSearch = _modalController.view;
    if ([_modalController isKindOfClass:[UINavigationController class]]) {
        viewToSearch = ((UINavigationController *)_modalController).topViewController.view;
    }
    for (UIScrollView *scrollView in viewToSearch.subviews) {
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    if (!self.isDismiss) {
        ...
        [self cornerRadiusForView:toView withRadius:HAS_ROUNDED_CORNERS?38.5:8.f];
        [self cornerRadiusForView:fromView withRadius:HAS_ROUNDED_CORNERS?38.5:8.f];
        
        toView.frame = CGRectMake(0, toView.frame.size.height, toView.frame.size.width, toView.frame.size.height);
        
        CGFloat fromViewAlpha = _behindViewAlpha;
        CGFloat fromViewScale = _behindViewScale;
        [UIView animateWithDuration:_transitionDuration delay:0 usingSpringWithDamping:0.85 initialSpringVelocity:0.5f options:UIViewAnimationOptionCurveEaseInOut animations:^{
            fromView.alpha = fromViewAlpha;
            fromView.transform = CGAffineTransformMakeScale(fromViewScale, fromViewScale);
        ...
        toView.alpha = _behindViewAlpha;
        toView.transform = CGAffineTransformMakeScale(_behindViewScale, _behindViewScale);
        fromView.alpha = 1;
        [self cornerRadiusForView:toView withRadius:HAS_ROUNDED_CORNERS?38.5:8.f];
        [self cornerRadiusForView:fromView withRadius:HAS_ROUNDED_CORNERS?38.5:8.f];
...
- (void)cornerRadiusForView:(UIView *)sender withRadius:(CGFloat)radius {
    /*
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        NSLog(@"velocity in the y direction: %f", velocity.y);
        ...
        CGFloat velocityForSelectedDirection = 0.0;
...
-(void)startInteractiveTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    if (![self isPriorToIOS8]) {
        toViewController.view.transform = CGAffineTransformMakeScale(self.behindViewScale, self.behindViewScale);
    }

    self.tempTransform = toViewController.view.transform;

    toViewController.view.alpha = self.behindViewAlpha;
    [self cornerRadiusForView:toViewController.view withRadius:HAS_ROUNDED_CORNERS?38.5:8.f];
    [self cornerRadiusForView:fromViewController.view withRadius:HAS_ROUNDED_CORNERS?38.5:8.f];
...
- (void)updateInteractiveTransition:(CGFloat)percentComplete
{
    ...
    UIViewController *fromViewController = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    UIViewController *toViewController = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    toViewController.view.transform = CGAffineTransformMakeScale(self.behindViewScale + ((1 - self.behindViewScale) * percentComplete), self.behindViewScale + ((1 - self.behindViewScale) * percentComplete));
...
- (void)cancelInteractiveTransition
{
    ...
    void (^animations)(void) = ^{
        toViewController.view.transform = self.tempTransform;
        toViewController.view.alpha = self.behindViewAlpha;
...
{
    if (self.direction == ZFModalTransitonDirectionBottom) {
        NSLog(@"yes yes yes yes yes allow it!!");
        return YES;
    }
    return NO;
}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    CGFloat topVerticalOffset = -self.scrollview.adjustedContentInset.top;
