...
- (BOOL)isMember {
    return [self.room.attributes.context.status isEqualToString:CAMP_STATUS_MEMBER];
}
- (BOOL)isAdmin {
    return self.room.attributes.context.membership.role.identifier == CAMP_ROLE_ADMIN;
}
- (BOOL)isPrivate {
...
- (void)approveRequest:(id)sender {
    ...
    NSString *url = [NSString stringWithFormat:@"camps/%@/members/requests", self.room.identifier];
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if ([s.identifier isEqualToString:@"members_requests"] &&
        ...
        [shareWithFriends setTitle:[NSString stringWithFormat:@"Share %@", self.room.attributes.details.title] forState:UIControlStateNormal];
        ...
        [shareWithFriends bk_whenTapped:^{
            [[Launcher sharedInstance] shareCamp:self.room];
    ...
    header.title = (s.title ? [s.title stringByReplacingOccurrencesOfString:@"{members_count}" withString:[NSString stringWithFormat:@"%ld", self.room.attributes.summaries.counts.members]] : @"");
    ...
    if ([[tableView cellForRowAtIndexPath:indexPath] isKindOfClass:[SearchResultCell class]] ||
        ...
        if (objectExists(indexPath.row, s.data)) {
            ...
            if ([s.data[indexPath.row] isKindOfClass:[User class]] || ([s.data[indexPath.row] objectForKey:@"type"] && [s.data[indexPath.row][@"type"] isEqualToString:@"user"])) {
                ...
                if ([s.identifier isEqualToString:@"members_directors"] || [s.identifier isEqualToString:@"members_moderators"]) {
                    UIAlertAction *removeRole = [UIAlertAction actionWithTitle:[NSString stringWithFormat:@"Remove as %@", [s.identifier isEqualToString:@"members_directors"] ? @"Director" : @"Manager"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                        if ([s.identifier isEqualToString:@"members_directors"]) {
                            [self removeManagerRole:RoomManagerTypeAdmin user:user];
                        }
                        else {
                            [self removeManagerRole:RoomManagerTypeModerator user:user];
                        }
                    }];
...
- (void)removeManagerRole:(RoomManagerType)managerType user:(User *)user {
    NSString *url = [NSString stringWithFormat:@"camps/%@/members/roles", self.room.identifier];
    
    // create the group
    JGProgressHUD *HUD = [JGProgressHUD progressHUDWithStyle:JGProgressHUDStyleExtraLight];
    HUD.textLabel.text = [NSString stringWithFormat:@"Removing as %@...", (managerType == RoomManagerTypeAdmin ? @"Director" : @"Manager")];
    ...
    NSDictionary *params;
    if (managerType == RoomManagerTypeAdmin) {
        params = @{@"user_id": user.identifier, @"role": @(2)};
    }
    else if (managerType == RoomManagerTypeModerator) {
        params = @{@"user_id": user.identifier, @"role": @(1)};
    }
    
    NSLog(@"params: %@", params);
    
    [[[HAWebService managerWithContentType:kCONTENT_TYPE_JSON] authenticate] DELETE:url parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"RoomManagersUpdated" object:self.room];
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        // on the completion of each request
