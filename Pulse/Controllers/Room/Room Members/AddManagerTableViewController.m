...
- (void)viewDidLoad {
    ...
    self.title = [NSString stringWithFormat:@"Add %@", (self.managerType == RoomManagerTypeAdmin ? @"Directors" : @"Managers")];
    self.view.tintColor = [UIColor fromHex:self.room.attributes.details.color];
...
- (void)getMembers {
    NSString *url = [NSString stringWithFormat:@"rooms/%@/members", self.room.identifier];
...
- (void)setupErrorView {
    self.errorView = [[ErrorView alloc] initWithFrame:CGRectMake(16, 0, self.view.frame.size.width - 32, 100) title:@"No Members Available" description:[NSString stringWithFormat:@"Have others join the Camp before assigning them %@", self.managerType == RoomManagerTypeAdmin ? @"directors" : @"managers"] type:ErrorViewTypeGeneral];
...
- (void)save {
    NSString *url = [NSString stringWithFormat:@"rooms/%@/members/assign", self.room.identifier];
    
    // create the group
    JGProgressHUD *HUD = [JGProgressHUD progressHUDWithStyle:JGProgressHUDStyleExtraLight];
    HUD.textLabel.text = [NSString stringWithFormat:@"Adding %@%@...", (self.managerType == RoomManagerTypeAdmin ? @"Admin" : @"Manager"), self.selectedMembers.count > 1 ? @"s" : @""];
    ...
    for (NSString *identifier in self.selectedMembers) {
        NSDictionary *params;
        if (self.managerType == RoomManagerTypeAdmin) {
            params = @{@"user_id": identifier, @"role": @(2)};
        }
        else if (self.managerType == RoomManagerTypeModerator) {
            params = @{@"user_id": identifier, @"role": @(1)};
        }
        ...
        dispatch_group_notify(group, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            ...
            HUD.textLabel.text = [NSString stringWithFormat:@"Added %@%@!", (self.managerType == RoomManagerTypeAdmin ? @"Admin" : @"Manager"), self.selectedMembers.count > 1 ? @"s" : @""];
