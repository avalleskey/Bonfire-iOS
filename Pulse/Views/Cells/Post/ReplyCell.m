...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        self.profilePicture.frame = CGRectMake(70, replyContentOffset.top, 28, 28);
        ...
        /*
         ...
         self.topLine.backgroundColor = [UIColor colorWithRed:0.95 green:0.95 blue:0.96 alpha:1.0];
...
- (void)layoutSubviews
{
    ...
    if (![self.moreButton isHidden]) {
        ...
        self.moreButton.frame = CGRectMake(self.frame.size.width - moreButtonWidth - replyContentOffset.right + moreButtonPadding, self.profilePicture.frame.origin.y - moreButtonPadding, moreButtonWidth, self.profilePicture.frame.size.height + (moreButtonPadding * 2));
    }
    
    self.nameLabel.frame = CGRectMake(offset.left, self.profilePicture.frame.origin.y, self.frame.size.width - offset.left - offset.right, 16);
    CGFloat yBottom = self.nameLabel.frame.origin.y + self.nameLabel.frame.size.height + 6;
    ...
    /*
     ...
     self.topLine.frame = CGRectMake(self.profilePicture.frame.origin.x + (self.profilePicture.frame.size.width / 2) - (self.topLine.frame.size.width / 2), - (self.topLine.layer.cornerRadius / 2), self.topLine.frame.size.width, self.profilePicture.frame.origin.y - profilePicPadding + (self.topLine.layer.cornerRadius / 2));
     
     self.bottomLine.hidden = self.bottomCell;
     if (![self.bottomLine isHidden]) {
     self.bottomLine.frame = CGRectMake(self.profilePicture.frame.origin.x + (self.profilePicture.frame.size.width / 2) - (self.bottomLine.frame.size.width / 2), self.profilePicture.frame.origin.y + self.profilePicture.frame.size.width + profilePicPadding, self.bottomLine.frame.size.width, self.frame.size.height - (self.profilePicture.frame.origin.y + self.profilePicture.frame.size.width + profilePicPadding) + (self.bottomLine.layer.cornerRadius / 2));
     }
     */
...
+ (CGFloat)heightForPost:(Post *)post {    
    ...
    CGFloat headerHeight = 16 + 6; // 3pt padding underneath
