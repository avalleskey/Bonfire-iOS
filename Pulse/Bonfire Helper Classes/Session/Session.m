...
+ (Session *)sharedInstance {
    if (!session) {
        ...
        // init AFNetworking session [HAWebService authenticatedManager]
...
- (void)syncDeviceToken {
    ...
    if (session.currentUser && [[NSUserDefaults standardUserDefaults] stringForKey:@"device_token"] && ![[[NSUserDefaults standardUserDefaults] stringForKey:@"device_token"] isEqualToString:[[NSUserDefaults standardUserDefaults] stringForKey:@"user_device_token"]]) {
        ...
        [[HAWebService authenticatedManager] POST:url parameters:@{@"vendor": @(1), @"token": [[NSUserDefaults standardUserDefaults] stringForKey:@"device_token"]} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
+ (void)authenticate:(void (^)(BOOL success, NSString *token))handler {
    ...
    // load cache of user
    if (accessToken != nil) {
        NSLog(@"duh token: %@", accessToken[@"attributes"][@"access_token"]);
