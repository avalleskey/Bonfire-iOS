...
- (void)setObject:(BFTipObject *)object {
    if (object != _object) {
        ...
        if (object.creatorType == BFTipCreatorTypeRoom) {
            self.creatorAvatarView.room = object.creator;
        }
        else if (object.creatorType == BFTipCreatorTypeUser) {
            self.creatorAvatarView.user = object.creator;
        }
        else {
            self.creatorAvatarView.room = nil;
...
- (void)setStyle:(BFTipViewStyle)style {
    if (style == BFTipViewStyleDark) {
        ...
        self.blurView.backgroundColor = [[UIColor bonfireBlack] colorWithAlphaComponent:0.3];
...
+ (BFTipObject *)tipWithCreatorType:(BFTipCreatorType)creatorType creator:(id _Nullable)creator title:(NSString * _Nullable)title text:(NSString *)text action:(void (^ __nullable)(void))actionHandler {
    ...
    switch (creatorType) {
        ...
        case BFTipCreatorTypeRoom: {
            if ([creator isKindOfClass:[Room class]]) {
                Room *roomCreator = (Room *)creator;
                tipObject.creator = roomCreator;
                tipObject.creatorText = [NSString stringWithFormat:@"%@", roomCreator.attributes.details.title];
