...
@property (strong, nonatomic) UIVisualEffectView *centerView;
...
- (void) createSubviews {
    ...
    _centerView = [[UIVisualEffectView alloc] initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleLight]];
    _centerView.frame = CGRectMake(0, 0, sampleItemView.frame.size.width, sampleItemView.frame.size.width);
    _centerView.layer.cornerRadius = _centerView.frame.size.height / 2;
    _centerView.layer.masksToBounds = true;
    _centerView.backgroundColor = [UIColor colorWithWhite:0.4f alpha:1];
...
- (void) setCenterViewHighlighted: (BOOL) highlighted {
    //_centerView.backgroundColor = highlighted ? [UIColor colorWithWhite:1 alpha:0.5] : [UIColor clearColor];
}
