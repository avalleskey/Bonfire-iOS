...
#define BOT_ATTACHMENT_EDGE_INSETS UIEdgeInsetsMake(24, 24, 16, 24)

// avatar macros
#define BOT_ATTACHMENT_AVATAR_SIZE 72
#define BOT_ATTACHMENT_AVATAR_BOTTOM_PADDING 8
// display name macros
#define BOT_ATTACHMENT_DISPLAY_NAME_FONT [UIFont systemFontOfSize:20.f weight:UIFontWeightHeavy]
#define BOT_ATTACHMENT_DISPLAY_NAME_BOTTOM_PADDING 3
// username macros
#define BOT_ATTACHMENT_USERNAME_FONT [UIFont systemFontOfSize:14.f weight:UIFontWeightBold]
#define BOT_ATTACHMENT_USERNAME_BOTTOM_PADDING 8
// bio macros
#define BOT_ATTACHMENT_BIO_FONT [UIFont systemFontOfSize:14.f weight:UIFontWeightMedium]
#define BOT_ATTACHMENT_BIO_BOTTOM_PADDING 12
// details macros
#define BOT_ATTACHMENT_DETAILS_EDGE_INSETS UIEdgeInsetsMake(12, 24, 10, 24)

@implementation BFBotAttachmentView
...
- (void)setup {
    ...
    self.headerBackdrop = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, BOT_ATTACHMENT_EDGE_INSETS.top + BOT_ATTACHMENT_AVATAR_SIZE / 2)];
    self.headerBackdrop.backgroundColor = [UIColor bonfireOrange];
    [self.contentView addSubview:self.headerBackdrop];
    
    self.avatarContainerView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.size.width / 2 - BOT_ATTACHMENT_AVATAR_SIZE / 2 - 4, BOT_ATTACHMENT_EDGE_INSETS.top - 4, BOT_ATTACHMENT_AVATAR_SIZE + 8, BOT_ATTACHMENT_AVATAR_SIZE + 8)];
    ...
    self.avatarView = [[BFAvatarView alloc] initWithFrame:CGRectMake(4, 4, BOT_ATTACHMENT_AVATAR_SIZE, BOT_ATTACHMENT_AVATAR_SIZE)];
    self.avatarView.userInteractionEnabled = false;
    [self.avatarContainerView addSubview:self.avatarView];
    
    // display name
    self.textLabel = [[UILabel alloc] init];
    self.textLabel.font = BOT_ATTACHMENT_DISPLAY_NAME_FONT;
    ...
    self.detailTextLabel.font = [UIFont systemFontOfSize:BOT_ATTACHMENT_USERNAME_FONT.pointSize weight:UIFontWeightHeavy];
    ...
    self.bioLabel.font = BOT_ATTACHMENT_BIO_FONT;
    ...
    self.detailsCollectionView = [[BFDetailsCollectionView alloc] initWithFrame:CGRectMake(BOT_ATTACHMENT_EDGE_INSETS.left, 0, [UIScreen mainScreen].bounds.size.width - BOT_ATTACHMENT_EDGE_INSETS.left - BOT_ATTACHMENT_EDGE_INSETS.right, 16)];
...
- (void)layoutSubviews {
    ...
    CGFloat bottomY = 0;
    CGFloat maxWidth = self.frame.size.width - (BOT_ATTACHMENT_EDGE_INSETS.left + BOT_ATTACHMENT_EDGE_INSETS.right);
    ...
    if (self.textLabel.attributedText.length > 0) {
        CGRect textLabelRect = [self.textLabel.attributedText boundingRectWithSize:CGSizeMake(maxWidth, CGFLOAT_MAX) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) context:nil];
            self.textLabel.frame = CGRectMake(BOT_ATTACHMENT_EDGE_INSETS.left, bottomY + BOT_ATTACHMENT_AVATAR_BOTTOM_PADDING, maxWidth, ceilf(textLabelRect.size.height));
    ...
    if (self.detailTextLabel.text.length > 0) {
        ...
        self.detailTextLabel.frame = CGRectMake(BOT_ATTACHMENT_EDGE_INSETS.left, bottomY + BOT_ATTACHMENT_DISPLAY_NAME_BOTTOM_PADDING, maxWidth, ceilf(detailLabelRect.size.height));
    ...
    if (hasBio) {
        ...
        self.bioLabel.frame = CGRectMake(BOT_ATTACHMENT_EDGE_INSETS.left, bottomY + BOT_ATTACHMENT_USERNAME_BOTTOM_PADDING, maxWidth, ceilf(bioLabelRect.size.height));
    ...
    if (hasDetails) {
        self.detailsCollectionView.frame = CGRectMake(BOT_ATTACHMENT_EDGE_INSETS.left, bottomY + (hasBio ? BOT_ATTACHMENT_BIO_BOTTOM_PADDING : BOT_ATTACHMENT_USERNAME_BOTTOM_PADDING), self.frame.size.width - (BOT_ATTACHMENT_EDGE_INSETS.left + BOT_ATTACHMENT_EDGE_INSETS.right), self.detailsCollectionView.collectionViewLayout.collectionViewContentSize.height);
...
- (void)setBot:(Bot *)bot {
    if (bot != _bot) {
        ...
        self.avatarView.bot = bot;
                        
        // display name
        self.textLabel.hidden = (bot.attributes.displayName.length == 0);
        if ([self.textLabel isHidden]) {
            self.textLabel.text = @"";
        }
        else {
            NSString *displayName = bot.attributes.displayName;
                        
            NSMutableAttributedString *displayNameAttributedString = [[NSMutableAttributedString alloc] initWithString:displayName attributes:@{NSFontAttributeName:BOT_ATTACHMENT_DISPLAY_NAME_FONT}];
            if ([bot isVerified]) {
                NSMutableAttributedString *spacer = [[NSMutableAttributedString alloc] initWithString:@" "];
                [spacer addAttribute:NSFontAttributeName value:BOT_ATTACHMENT_DISPLAY_NAME_FONT range:NSMakeRange(0, spacer.length)];
                ...
                [attachment setBounds:CGRectMake(0, roundf(BOT_ATTACHMENT_DISPLAY_NAME_FONT.capHeight - attachment.image.size.height)/2.f-1, attachment.image.size.width, attachment.image.size.height)];
        ...
        self.detailTextLabel.hidden = (bot.attributes.identifier.length == 0);
        ...
        if ([self.bioLabel isHidden]) {
            ...
            [attrString addAttribute:NSFontAttributeName value:BOT_ATTACHMENT_BIO_FONT range:NSMakeRange(0, attrString.length)];
...
+ (CGFloat)heightForBot:(Bot *)bot width:(CGFloat)width {
    ...
    CGFloat maxWidth = (width - (BOT_ATTACHMENT_EDGE_INSETS.left + BOT_ATTACHMENT_EDGE_INSETS.right));
    
    // knock out all the required bits first
    CGFloat height = BOT_ATTACHMENT_EDGE_INSETS.top + BOT_ATTACHMENT_AVATAR_SIZE + BOT_ATTACHMENT_AVATAR_BOTTOM_PADDING;
    ...
    if (bot.attributes.identifier.length > 0) {
        CGRect usernameRect = [[NSString stringWithFormat:@"@%@", bot.attributes.identifier] boundingRectWithSize:CGSizeMake(maxWidth, CGFLOAT_MAX) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName:BOT_ATTACHMENT_USERNAME_FONT} context:nil];
        CGFloat usernameHeight = ceilf(usernameRect.size.height);
        height += BOT_ATTACHMENT_DISPLAY_NAME_BOTTOM_PADDING + usernameHeight;
    }
    ...
    if (bot.attributes.theDescription.length > 0) {
        ...
        height += BOT_ATTACHMENT_USERNAME_BOTTOM_PADDING + bioHeight;
    }
    
    if (bot.identifier.length > 0) {
        NSMutableArray *details = [[NSMutableArray alloc] init];
        if (bot.attributes.website.displayUrl.length > 0) {
            BFDetailItem *item = [[BFDetailItem alloc] initWithType:BFDetailItemTypeWebsite value:bot.attributes.website.displayUrl action:nil];
            [details addObject:item];
        ...
        if (details.count > 0) {
            BFDetailsCollectionView *detailCollectionView = [[BFDetailsCollectionView alloc] initWithFrame:CGRectMake(BOT_ATTACHMENT_EDGE_INSETS.left, 0, width - BOT_ATTACHMENT_EDGE_INSETS.left - BOT_ATTACHMENT_EDGE_INSETS.right, 16)];
            ...
            height = height + (bot.attributes.theDescription.length > 0 ? BOT_ATTACHMENT_BIO_BOTTOM_PADDING : BOT_ATTACHMENT_USERNAME_BOTTOM_PADDING) +  BOT_ATTACHMENT_DETAILS_EDGE_INSETS.top + detailCollectionView.collectionViewLayout.collectionViewContentSize.height;
        }
    }
    
    return height + BOT_ATTACHMENT_EDGE_INSETS.bottom;
}
