...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadCampManagers:) name:@"CampManagersUpdated" object:nil];
}
...
- (void)reloadCampManagers:(NSNotification *)notification {
    ...
    if ([[tableView cellForRowAtIndexPath:indexPath] isKindOfClass:[SearchResultCell class]] ||
        ...
        if (objectExists(indexPath.row, s.data)) {
            ...
            if ([s.data[indexPath.row] isKindOfClass:[User class]] || ([s.data[indexPath.row] objectForKey:@"type"] && [s.data[indexPath.row][@"type"] isEqualToString:@"user"])) {
                ...
                if ([self isAdmin] && ([s.identifier isEqualToString:@"members_admin"] || [s.identifier isEqualToString:@"members_moderator"])) {
...
//                UIAlertAction *debugUser = [UIAlertAction actionWithTitle:@"Debug" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//                    [Launcher openDebugView:user];
//                }];
//                [actionSheet addAction:debugUser];
